...
[@Van_den_Berge:2019] is an interesting review describing RNA-seq
...
counts_result <- read_tsv(file = "data/sample1_counts.tsv.gz")
counts_result
```
...
Here is an example of a `featureCounts` output file run on 6 BAM files. The last
6 columns of the table give the raw counts for each gene in each sample.
You can download it
[here](https://raw.githubusercontent.com/UCLouvain-BIOINFO/bioinfo-training-03-rnaseqdata/data/all_samples_counts.tsv.gz).
