...
3. Inspect the counts. What is the maximum value? How many genes have
   zero counts ?
...
results can be merged in a single count matrix, were each column
represents a sample, and each line represents a gene. This count
matrix will be the starting point of a differential expression
analysis.

```{r, echo = FALSE}
read.csv("data/count_matrix.csv", row.names = 1)[100:106, 1:6]
```
...
generated by FeatureCounts located in the `data` folder. Use these
files to create a count matrix.
