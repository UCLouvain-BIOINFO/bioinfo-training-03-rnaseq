...
episode, we will focus on two widely used methods: the over-representation analysis
...
We will use the results of the differential expression analysis comparing KD to 
mock epithelial cells. The following code summarizes the full DESeq2 analysis 
that you should already have learned about in the previous episode. 
We will focus on the genes that have an adjusted p-value (those that have been 
tested).
...
dds <- DESeq(dds)
res <- results(dds, name = "group_Epithelial_KD_vs_Epithelial_mock")
...
Gene Ontology terms, KEGG pathways, or Reactome pathways), i.e, if it contains 
more genes from these categories than would be expected by chance.
...
fig3 <- res_tbl %>%
  ...
  geom_vline(xintercept = log2FC_thr) +
  geom_vline(xintercept = -log2FC_thr) +
  ggtitle(paste0(length(genes_DE3), " DE selected (padj < ", padj_thr, ' and |logFC| > ', log2FC_thr, ""))

fig1 + fig2 + fig3
```

We could consider all genes that have a p-adjusted value < 0.05 (left volcano),
in this case we would select `r length(genes_DE1)` genes. This number is probably 
too large, so we should probably be more restrictive and select a smaller set of 
genes, using a more stringent filtering on the p-adjusted value. 
We could also consider filtering using the logFC, which could also make 
biological sense.


Whatever thresholds are used, the idea here is to start from a selection of 
genes that we will consider as our DE genes. 
**The selection is a choice the user has to make.**
...
are in that category, and how many are not.
...
res_tbl %>%
  ...
  geom_point(data = res_tbl %>% filter(gene %in% genes_from_geneset), color = "blue", size = 1) +
  theme(legend.position = "none") +
  geom_hline(yintercept = -log10(padj_thr)) +
  geom_vline(xintercept = log2FC_thr) +
  geom_vline(xintercept = -log2FC_thr) +
...
ORA_res_tbl_GO <- as_tibble(ORA_res) %>% filter(p.adjust < 0.05)
ORA_res_tbl_GO %>%
...
You may have noticed the total number of DE genes changes. We defined `r length(genes_DE)` DE genes,
but only `r length(genes_DE[genes_DE %in% GO_BP$gene_symbol])` DE genes are
included in the enrichment result table (in the GeneRatio column).
Similarly, our universe had `r length(geneList)` genes, but the total number of tested genes was
only `r nrow(res_tbl %>% filter(gene %in% GO_BP$gene_symbol))` in the result table.
...
Let's inspect genes from the `GOBP_RESPONSE_TO_TRANSFORMING_GROWTH_FACTOR_BETA`on the volcano
...
Test a different threshold to define the group of 'DE genes' and check if, 
in the cases above, this has and effect on the GO terms of interest.
...
ORA_res_GO_new_thr <- as_tibble(ORA_res_new_thr) %>% filter(p.adjust < 0.05)
ORA_res_GO_new_thr %>%
  select(ID, GeneRatio, BgRatio, pvalue, p.adjust)
