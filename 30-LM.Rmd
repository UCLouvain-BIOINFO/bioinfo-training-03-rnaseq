...
- treatments effects (these are likely the effects you actually want
  to study)
...
A linear model provides a mathematical framework to describe and quantify the
contribution of these different factors to the observed outcome.
...
groups of our experiment. It will take the value of 0 when considering the
...
### Estimating coefficients and testing their significance

$\beta_0$ and $\beta_1$ coefficients can be estimated from the data using
...
We are going to use a subset of our [previous
dataset](https://github.com/UCLouvain-BIOINFO/bioinfo-training-03-rnaseq/raw/refs/heads/main/data/se.rds)
as a toy example.  Let's focus only on the gene *ZBED2* in
*Epithelial* cell type, and use the log-transformed expression values.
...
modeled in the different samples:
...
Epithelial and Fibroblasts count values are mixed,
even though they clearly don’t start at the same baseline.
...
model a treatment effect that would differ across cell types.
