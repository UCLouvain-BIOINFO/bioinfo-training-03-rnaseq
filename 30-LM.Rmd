...
-	the treatments effects (these are be the effects you actually want to study)
...
Let's focus only on the gene *ZBED2* in *Epithelial* cell type.


```{r, eval=TRUE, purl=TRUE, message=FALSE, warning = F}
se <- readRDS("data/se.rds")

gene_count <- as_tibble(assay(se["ZBED2", se$Type == "Epithelial"]), rownames = "gene") %>% 
  pivot_longer(names_to = "sample", values_to = "count", -gene) %>% 
  left_join(as_tibble(colData(se)))
```

Let's also convert the `Condition` column into a factor, in order to set the `mock` 
condition as the reference level. 

```{r}
gene_count$Condition <- factor(gene_count$Condition, levels = c("mock", "KD"))
```

And finally, let’s log-transform the count values.
...
as we’ll see later, this isn’t actually true for RNAseq data!) and let's use the 
`lm()` function to fit a linear model to compare KD to mock cells.
...
- The estimate for the $\beta_1$ coefficient (annotated as the `ConditionKD` coefficient) 
is `r round(coefficients(mod1)[2], 3)`
...
gene_count %>% 
  ...
  geom_point(aes(x = "mock", y = coefficients(mod1)[1]), color = "blue", size = 4) +
  geom_point(aes(x = "KD", y = coefficients(mod1)[1] + coefficients(mod1)[2]), color = "red", size = 4)
```

And indeed, we see that

-- the intercept (the $\beta_0$ coefficient) corresponds to the mean expression 
level of the gene in the `mock` group (the blue dot)

-- the sum of $\beta_0$ and $\beta_1$ coefficients (the intercept + the logFC)
corresponds to the mean expression level of the gene in the `KD` group
(the red dot)


We can also extract the p-value associated with the `ConditionKD` coefficient:
...
conducted across two different batches (batch1 and batch2), with 3 control and 
3 treated samples in each batch.

To take into account the treatment effect while controlling the batch effect, 
we should use the following design: $$design \sim Treatment + Batch$$ 
...
$$y = \beta_0 + Treatment. x_1 + Batch . x_2$$


In this equation, 

- the $\beta_0$ term represents the `intercept`, i.e, the **expression level of the gene 
in the reference sample**, for instance the untreated cells from batch1.
...
batches of our experiment. It will take the value of 0 if we are considering 
batch1, and a value of 1 when considering batch2.

- The `Batch` term represents the **logFoldchange associated with the batch effect**.
...
coldata <- tibble(sample = paste0("sample", 1:12),
                  Batch = c(rep("Batch1", 6), rep("Batch2", 6)),
...
- In `untreated cells from Batch1`: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$ = $\beta_0$ 

- In `treated cells from Batch1`: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$ = $\beta_0$ + *Treatment*

- In `untreated cells from Batch2`: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$ = $\beta_0$ + *Batch*

- In `treated cells from Batch2`: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$ = $\beta_0$ + *Treatment* + *Batch*
...
Let's also convert the `Condition` column into a factor, in order to set the `mock` 
condition as the reference level, and log-transform the count values.
...
We will start by fitting a linear model that only accounts for the *Condition* effect,
ignoring the *cell type* effect.
...
Epithelial and Fibroblasts count values are mixed, even though they clearly 
don’t start at the same baseline.
...
Use the `lm()` function to fit a linear model that would now account for 
...
The coefficients `ConditionKD` (representing the *KD Effect*) and
the coefficient `TypeFibroblast`, representing the *cell type Effect*, are both significant.
...
The model now adjusts for the baseline difference between epithelial and fibroblast cell types. 

It prevents cell type differences from biasing the condition effect.
...
and 3 treated samples across 2 different batches (batch1 and batch2). 
An interaction term could be added to the linear model, to modelise a 
treatment effect that would differ across batches.

Our design would hence be :
$$design \sim Treatment*Batch$$
...
$$y = \beta_0 + Treatment. x_1 + Batch . x_2 + Treatment.Batch.x_1.x_2$$


In this equation, 

- the $\beta_0$ term represents the `intercept`, i.e, the **expression level of the gene 
in the reference sample**, for instance the untreated cells in batch1.

- the design factor $x_1$ will take the value of  0 if we are considering 
untreated samples, and a value of 1 when considering treated samples.

- The `Treatment` term represents the **treatment effect in the reference batch**.
In this case it would represent the logFoldchange between treated and untreated cells in batch1.

- the design factor $x_2$  will take the value of 0 if we are 
considering batch1, and a value of 1 when considering batch2.

- The `Batch` term represents the **logFoldchange associated with the batch effect in the reference treatment**.
In this case it would represent the batch effect in untreated cells.

- The `Treatment.Batch` term represents the **treatment effect that would be different in batch2 compared to batch1**. 
This term will only be considered in the equation when we are referring to treated cells from batch2 (i.e. when $x_1$ = 1 and $x_2$ = 1)
...
- In `treated cells from batch2`: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$ = $\beta_0$ + *Treatment* + *Batch* + *Treatment.Batch* 
...
```{r, eval=TRUE, purl=TRUE, message=FALSE}
...
The interaction term is significant. This indicates that the *KD effect* 
is significantly **different** in epithelial cells and fibroblasts.

And indeed looking at the count values, we can see that the KD increases
the gene expression level in epithelial cells, but it has no effect in fibroblasts.
...
The coefficients `ConditionKD` (representing the *KD Effect*) is significant, 
but the interaction term is not. This indicates that the *KD Effect* is not
significantly different between fibroblasts and epithelial cells.
