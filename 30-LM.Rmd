...
gene_count |>
    ...
    geom_point(aes(x = "mock", y = coefficients(mod1)[1]),
               color = "blue", size = 4) +
...
experiment was conducted across two different batches (batch1 and
batch2), with 3 control and 3 treated samples in each batch.

To take into account the treatment effect while controlling the batch
...
- The $\beta_0$ term represents the `intercept`, i.e, the **expression
  level of the gene in the reference sample**, for instance the
  untreated cells from batch1.
...
- The design factor $x_2$ is another binary indicator variable
  representing the two batches of our experiment. It will take the
  value of 0 if we are considering batch1, and a value of 1 when
  considering batch2.
- The `Batch` term represents the **logFoldchange associated with the
  batch effect**.
...
- In `untreated cells from Batch1`:
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$ =
  $\beta_0$
- In `treated cells from Batch1`:
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$
  = $\beta_0$ + *Treatment*
- In `untreated cells from Batch2`:
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$ =
  $\beta_0$ + *Batch*
- In `treated cells from Batch2`:
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$
  = $\beta_0$ + *Treatment* + *Batch*
...
controls and 3 treated samples across 2 different batches (batch1 and
batch2).  An interaction term could be added to the linear model, to
modelise a treatment effect that would differ across batches.

Our design would hence be: $$design \sim Treatment*Batch$$
...
- Rhe $\beta_0$ term represents the `intercept`, i.e, the **expression
  level of the gene in the reference sample**, for instance the
  untreated cells in batch1.
- The design factor $x_1$ will take the value of 0 if we are
  considering untreated samples, and a value of 1 when considering
  treated samples.
- The `Treatment` term represents the **treatment effect in the
  reference batch**.  In this case it would represent the
  logFoldchange between treated and untreated cells in batch1.
- The design factor $x_2$ will take the value of 0 if we are
  considering batch1, and a value of 1 when considering batch2.
- The `Batch` term represents the **logFoldchange associated with the
  batch effect in the reference treatment**.  In this case it would
  represent the batch effect in untreated cells.
- The `Treatment.Batch` term represents the **treatment effect that
  would be different in batch2 compared to batch1**.  This term will
  only be considered in the equation when we are referring to treated
  cells from batch2 (i.e. when $x_1$ = 1 and $x_2$ = 1)
...
- In `treated cells from batch2`:
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$y$
  = $\beta_0$ + *Treatment* + *Batch* + *Treatment.Batch*
...
The interaction term is significant. This indicates that the *KD
effect* is significantly **different** in epithelial cells and
fibroblasts.
