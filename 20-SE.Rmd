...
coldata <- as_tibble(coldata, rownames = "sample") |>
    dplyr::select(-Cell)
coldata$sample <- c(paste0("Epi_mock", 1:3), paste0("Epi_KD", 1:3),
                    paste0("Fib_mock", 1:3), paste0("Fib_KD", 1:3))
...
se2 <- se[rowData(se)$chr %in% c("X", "Y"), 
          se$Type == "Fibroblast" & se$Condition == "mock"]
head(assay(se2))
rowData(se2)
```
...
Create a "group" column to to the metadata (i.e "Epithelial_mock", "Epithelial_KD", ...)
