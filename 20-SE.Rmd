...
rowdata <- tibble(gene = counts$gene,
       ...
       gene_biotype = mapIds(org.Hs.eg.db, ENSEMBL, "GENETYPE", "ENSEMBL")) |>
...
- **An expression matrix**: we load the count matrix, specifying that
  ...
  [here](https://raw.githubusercontent.com/UCLouvain-BIOINFO/bioinfo-training-03-rnaseqdata/data/count_matrix.csv).
...
- **A table describing the samples**, available
  [here](https://raw.githubusercontent.com/UCLouvain-BIOINFO/bioinfo-training-03-rnaseqdata/sample_metadata.csv).
...
- **A table describing the genes**, available
  [here](https://raw.githubusercontent.com/UCLouvain-BIOINFO/bioinfo-training-03-rnaseqdata/gene_metadata.csv).
...
Create a new object keeping only protein-coding genes (see `gene_biotype` feature
variable) and the Epithelial samples.

<details>
<summary>Solution</summary>

```{r}
se1 <- se[!is.na(rowData(se)$gene_biotype) & rowData(se)$gene_biotype == "protein-coding",
          colData(se)$Type == "Epithelial"]
se1
assay(se1)
colData(se1)
rowData(se1)
...
Extract the gene expression levels of the 3 first genes in Fibroblast mock cells

<details>
<summary>Solution</summary>

```{r, purl=FALSE}
assay(se)[1:3, colData(se)$Type == "Fibroblast" & colData(se)$Condition == "mock"]

# Equivalent to
assay(se)[1:3, se$Type == "Fibroblast" & se$Condition == "mock"]
```
