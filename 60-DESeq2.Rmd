...
- to understand the different theorical concepts behind a differential
  expression analysis;
- to provide a real-life example of differential expression (DE)
  analysis analysis running *DESeq2* step-by-step.


## Theory behind DESeq2

A large number of computational methods have been developed for
differential expression analysis [@Seyednasrollah:2015], differing
slightly in their methodology. Here we will present DESeq2, a widely
used bioconductor package dedicated to this type of analysis. For more
information read the original paper [@Huber:2014] and the [DESeq2
...
The starting point of the analysis is a count matrix, and the goal is
to identify genes that are differentially expressed between samples.

The different steps of the analysis are illustrated in the figure
below. Briefly, *DESeq2* starts by estimating scaling factors. Then,
it estimates the gene-wise dispersions and shrinks these estimates to
generate more accurate estimates of dispersion to model the
counts. Finally, *DESeq2* fits a generalized linear model, performs
hypothesis testing and generates a list of differentially expressed
genes.
...
As seen in the [WSBIM1322
course](https://uclouvain-cbio.github.io/WSBIM1322) with the example
of the coin toss, count data are often modelised by a binomial
...
$n$ independent experiments. In an RNA-seq experiment, $p$ would be
the probability of getting a read associated to a particular gene
given that $n$ total number of reads were sequenced in the
...
to RNA-seq. Indeed, for each sample, the total number of reads tends
to be in millions, while the counts per gene can vary considerably but
...
$$K_{ij} \sim P(\mu_{ij} = \sigma^2_{ij})$$

This assumption is, however, not valid for RNA-seq data, because the observed
variances tend to be higher than the means. RNA-seq data are said to be
**overdispersed** because counting noise is not the only source of variability.
Observed counts for each gene across biological replicates tend to fluctuate
more than expected due to the combination of biological and technical factors.
Applying a Poisson distribution to RNA-seq counts would underestimate the
variance, resulting in a high number of false positives.
...
to the $\mu$ parameter, an extra parameter $\alpha$ called the **dispersion**
...
Dispersion is a measure of variability in the data ($\alpha = CV^2$).
...
*DESeq2* fits a generalized linear model of the form: $$log2(q_{ij}) = \Sigma x_j.\beta_i$$

where the parameter $q_{ij}$ is proportional to the expected true
concentration of gene $i$ for sample $j$:

$$ q_{ij} = \frac{\mu_{ij}}{SizeFactor_{j}} $$

and $\beta_i$ represents the log2FC between conditions. $\beta_i$
coefficients are computed from the data.
...
chapter 7 from [WSBIM1322
course](https://uclouvain-cbio.github.io/WSBIM1322/).
...
We will run a *DESeq2* analysis using the real dataset that we stored
previously as a SummarizedExperiment in our `se` object.  Remember
this dataset corresponds to RNA-seq data from two different cell types
treated or not by a siRNA.

In case you forgot to save it previously, you can download it [here](https://raw.githubusercontent.com/UCLouvain-BIOINFO/bioinfo-training-03-rnaseq/refs/heads/main/data/se.rds).
...
We now switch our previous `SummarizedExperiment` object over to a
`DESeqDataSet` which is a subclass of `RangedSummarizedExperiment`
used by the *DESeq2* package to store the read counts and the
intermediate estimated quantities during statistical analysis.

The `DESeqDataSet` class enforces non-negative integer values in the
count matrix. In addition, a formula which specifies the design of the
experiment (the variables that will be used in modeling) must be
provided.
...
How many genes are not expressed in the dataset (zero counts in all samples)?

<details>
<summary>Solution</summary>

```{r}
table(rowSums(assay(dds)) > 0)
```

</details>

The count matrix contains many rows with only zeros. We can start by
removing these rows of the dds object as anyway no statistical test
can be done on these genes that have no variation in their counts
(equal to 0 in all samples).

It is not necessary to pre-filter low count genes before running the
`DESeq()` function, but by removing rows containing only zeros, we
reduce the memory size of the `dds` object and increase the speed of
the analysis.  Additional filtering to improve power will be applied
later.
...
single function `DESeq()`. This function will automatically run the
following other functions:

- `estimateSizeFactors()`:  estimation of size factors.
- `estimateDispersions()`: estimation of dispersion.
- `nbinomWaldTest()`: Negative Binomial GLM fitting and Wald
  statistics.
...
It is highly recommended to starts by a PCA to have a global view on
the data.  It will allow us to assess:

- Which samples are similar to or different from each other
- Whether the PCA fits the expectations from the experimental design
- Whether there are any sample outliers that may need further
  investigation
- The best design we should use to model the data


To perform PCA, we first need to transform the data.

This is important because RNA-seq data are intrinsically
heteroscedastic.  In other words, the variance is not constant across
the range of expression values.  Genes with higher expression levels
generally exhibit greater variance (left panel in the figure
below). Therefore, if raw counts are used as input for PCA, the result
will be dominated by highly expressed genes, as these genes contribute
most to the overall variance.

A simple strategy to address this issue would be to take the logarithm
of the normalised counts. However, log-transformed counts still
exhibit heteroscedasticity (middle panel).

As a solution, *DESeq2* offers the varianceStabilizingTransformation ,
`vst()`, which stabilizes the variance across the range of mean
expression values (right panel). For genes with high counts, the
transformation does not differ much from an ordinary log2
transformation. For genes with low expression, it applies stronger
variance stabilization, which reduces the high variability associated
with small counts and Poisson noise.  See `?vst` for more details
about the function.

```{r log-norm-vst-plots, echo=FALSE, warning=FALSE}
dds_group1 <- dds[, dds$group == "Epithelial_ctl"]
log_dds_group1 <- normTransform(dds_group1)
vst_dds_group1 <- vst(dds_group1)


fig1 <- tibble(meanCounts = rowMeans(counts(dds_group1, normalized = TRUE)),
         Variance = rowVars(counts(dds_group1, normalized = TRUE))) |>
    ggplot(aes(x = meanCounts, y = Variance)) +
...
fig2 <-  tibble(meanCounts = rowMeans(assay(log_dds_group1)),
         Variance = rowVars(assay(log_dds_group1))) |>
    ggplot(aes(x = meanCounts, y = Variance)) +
    ...
    theme(aspect.ratio = 1,
          ...
          title = element_text(size = 8))+
    ggtitle("logNormalized counts") +
    ylim(0, 6)

 fig3 <- tibble(meanCounts = rowMeans(assay(vst_dds_group1)),
         Variance = rowVars(assay(vst_dds_group1))) |>
    ggplot(aes(x = meanCounts, y = Variance)) +
    geom_point(alpha = 0.4, size = .2) +
    theme_bw() +
    theme(aspect.ratio = 1,
...
Let's apply the vst transformation on our `dds` object

```{r rlog}
vsd <- vst(dds)
```

The `plotPCA()` function can then be used on the transformed counts to
generate a PCA.
...
Note that the argument `returnData = TRUE` can be used to obtain a
dataframe of PC1 and PC2 for custom plotting.

```{r plotPCA-return-data}
pca_data <- plotPCA(vsd, intgroup = "group", returnData = TRUE)

ggplot(pca_data, aes(x = PC1, y = PC2, color = Condition, shape = Type)) +
  geom_point(size = 3) +
  ggrepel::geom_text_repel(aes(label = sample), color = "black", size = 2) +
  theme_bw() +
...
It is also advisable to investigate any systematic bias in the
sequencing data, such as whether one sample has been sequenced more
deeply than others. One can extract size factors using the
`sizeFactors()` function.  Usually these size factors should vary
around 1, indicating comparable sequencing depth.
...
Compare Size Factors to sequencing depths.
...
Plotting the dispersion estimates is a useful diagnostic.  This
dispersion plot is typical, with the final dispersion, estimates
shrunk from the gene-wise estimates towards the fitted estimates.
...
Try to reproduce the previous dispersion plot using values from the
`rowData` of the `dds` object.

Hints:

- The means of normalized counts are stored in the `baseMean` column.
- Gene initial dispersion values are stored in the `dispGeneEst`
  column.
- Gene final dispersion values are stored in the `dispersion` column.
- the dispersion-mean dependence are stored in the `dispFit` column.

<details>
<summary>Solution</summary>

```{r dispersion-plot-ex}
x <- as_tibble(rowData(dds)[, c("baseMean", "dispGeneEst", "dispFit", "dispersion",
                                "dispOutlier", "dispMAP")], rownames = "gene")

x |>
  ggplot(aes(x = baseMean, y = (dispGeneEst))) +
  geom_point(size = .5)  +
  geom_point(data = x, aes(x = (baseMean), y = (dispMAP)),
             size = .5, color = "dodgerblue")+
  geom_point(data = x, aes(x = (baseMean), y = (dispFit)),
             size = .5, color = "red") +
  scale_x_log10() +
  scale_y_log10()
```
...
The `resultsNames()` function gives the names of results that can be
extracted.
...
Note that by default, R will choose a reference level for factors
based on alphabetical order. Here `Epithelial_ctl` was hence set
arbitrarily as the reference level of the group factor.

```{r}
dds$group
```

Results can then be extracted using the `results()` function.  The
`name` parameter must be an element of resultsNames(object) specifying
the samples to compare.

Let's say we want to test the **KD effect in epithelial cells**.

```{r}
res <- results(dds,
               name = "group_Epithelial_KD_vs_Epithelial_ctl")
```


Note that the `contrast` argument of `results()` can also be used to
extract results of interest. In this case, the two following commands
will give the same results[^contrast].

[^contrast]: Using the `name` argument is however necessary to extract
specific coefficients from more complex designs.

```{r}
res <- results(dds,
               contrast = c("group", "Epithelial_KD", "Epithelial_ctl"))
```

##### Question {-}


How would you use the results() function to test the KD effect in
fibroblast cells? Try to do it in two different ways: one using the
`contrast` argument and one using the `name` argument.

<details>
<summary>Solution</summary>

1. With the `contrast` argument:

```{r}
results(dds,
        contrast = c("group", "Fibroblast_KD", "Fibroblast_ctl"))
```


2. With the `name` argument:

Using the resultsNames(), we can see that there is not coefficient
corresponding to `group_Fibroblast_KD_vs_Fibroblast_ctl`. This is not
surprising because the reference level of the group factor is
`Epithelial_ctl`.
...
In this case, we should first set the `Fibroblast_ctl` group as the
reference group. Reference levels can be changed using the `relevel()`
function. But in this case don't forget to re-run `DESeq()` function
after the re-leveling operation!


```{r message=FALSE, warning=FALSE}
dds_bis <- dds
dds_bis$group <- relevel(dds_bis$group, ref = "Fibroblast_ctl")
dds_bis <- DESeq(dds_bis)
resultsNames(dds_bis)
results(dds_bis, name = "group_Fibroblast_KD_vs_Fibroblast_ctl")
```
...
- **baseMean**: The average of the normalised count values taken over
  all samples.
- **log2FoldChange**: Fold-change between the comparison and control
  groups, reported on a logarithmic scale to base 2. In this case, as
  we are testing the "Condition_KD_vs_ctl" coefficient, positive
  log2FC indicates a gene up-regulated in the KD condition compared to
  the control condition, while a negative log2FC indicate a
  down-regulation.
- **lfcSE**: The standard error estimate for the log2FoldChange
  estimate.
- **stat**: The value of the test statistic for the gene.
- **pvalue**: The p-value of the test for the gene.
- **padj**: The p-value adjusted for multiple testing.

##### Question {-}

Inspect the results table and identify the "best gene" (showing the
lowest padjusted value).
...
Calculate the mean expression level of the "best gene" using
...
rowMeans(counts(dds[best_gene], normalized = TRUE))
```

##### Question {-}

Extract the ß coefficient of these "best gene" from the GLM
...
Using the function `counts()`, evaluate the mean expression level
of the "best gene" in control cells. Compare with ß coefficients.

<details>
<summary>Solution</summary>

```{r}
# log2 expression level in control cells
log2(rowMeans(counts(dds[best_gene, dds$group == "Epithelial_ctl"], normalized = TRUE)))
```
...
Evaluate the mean expression level of the "best gene"in KD cells.
...
```{r}
# log2 expression level in KD cells
log2(rowMeans(counts(dds[best_gene, dds$group == "Epithelial_KD"], normalized = TRUE)))
```

```{r}
# log2 expression level in KD cells evaluated from ß coefficients
coefficients(dds[best_gene])[,1] + coefficients(dds[best_gene])[,2]
```
...
This means that genes whith basemean < `r round(metadata(res)$filterThreshold,2)`
have been filtered. This represents
`r round(as.numeric(sub("%", "", names(metadata(res)$filterThreshold))), 1)` of
all tested genes!
...
The threshold chosen (red vertical line) is the lowest quantile for which the
number of rejections is within 1 residual standard deviation to the peak of the
curve.
...
1. Re-run the results() function on the same dds object, but set the
  independent filtering parameter to FALSE.  Check how many genes have
  no padj?
...
res_no_IF_tbl <- as_tibble(res_no_IF, rownames = "gene")
...
res_bis <- results(dds_bis,
                   name = "group_Epithelial_KD_vs_Epithelial_ctl",
                   independentFiltering = FALSE)
res_bis_tbl <- as_tibble(res_bis, rownames = "gene")

# Compare with previous analysis
# (Independant filtering threshold fixed automatically by DESeq2)
res_tbl |> filter(padj < 0.05) |> nrow()
res_bis_tbl |> filter(padj < 0.05) |> nrow()
```
...
It can give you an immediate idea of the proportion of genes differentially
expressed, (the taller the is the left peak, the more p-values are close to 0
and therefore significant). Furthermore, it gives an idea of how the test
behaved across all your hypotheses, and immediately diagnoses some potential
problems.

```{r pval-dist}
res_tbl |>
  filter(!is.na(padj)) |> # remove genes that were filtered out by IF
  ggplot(aes(x = pvalue)) +
  geom_histogram()
```
...
Drawing a volcano-plot is highly informative to have a global view on the results.
...
res_tbl |>
  ...
  geom_point(size = 0.5) +
  ggrepel::geom_text_repel(data = res_tbl |> filter(-log10(padj) > 30),
                           aes(label = gene),
                           color = "black")+
...
as_tibble(counts(dds[best_genes, ], normalized = TRUE), rownames = 'gene') |>
  pivot_longer(names_to = "sample", values_to = "counts", -gene) |>
  left_join(as_tibble(colData(dds))) |>
  ggplot(aes(x = sample, y = counts, fill = group)) +
...
1. Inspect counts of genes "CASC18" and "PRUNE1" plotted in red in the following
volcano-plot. What is the most striking?

```{r echo=FALSE, warning=FALSE}
selected_genes <- c("CASC18", "PRUNE1")

res_tbl |>
  ggplot(aes(x = log2FoldChange, y = -log10(padj),
             color = c(gene %in% selected_genes))) +
  scale_colour_manual(values = c("gray40", "red")) +
  geom_point(size = 0.5) +
  ggrepel::geom_text_repel(data = res_tbl |> filter(gene %in% selected_genes),
                           aes(label = gene),
                           color = "red")+
  geom_hline(yintercept = -log10(0.05)) +
  geom_vline(xintercept = 0) +
...
2. Using the `dispersions()` function, inspect the dispersion values of these genes.
...
as_tibble(counts(dds[selected_genes, ], normalized = TRUE),
          rownames = 'gene') |>
...
```{r}
x <- dispersions(dds[selected_genes,])
...
```{r lfc-shrink, warning = FALSE, message = FALSE}
res_shr <- lfcShrink(dds, coef = "group_Epithelial_KD_vs_Epithelial_ctl")
res_shr_tbl <- as_tibble(res_shr, rownames = "gene") |>
  filter(!is.na(padj))
...
fig2 <- ggplot(res_shr_tbl, aes(x = baseMean, y = log2FoldChange)) +
  ...
  ggtitle("MA plot with shrunken logFC values")
...
Below, we save the result table for next chapter to be used in the
next chapter.

```{r write-csv-res}
write_csv(res_tbl, file = "data/deseq_result_table.csv")
```
...
Re-run the analysis to compare fibroblast and epithelial cells
(in control condition)

### Question {-}

Which genes are affected differently by the siRNA in epithelial and
fibroblast cells?
...
- The
  ...
  package that can also be applied for RNA-Seq differential expression
  analysis [@edgeR:2025].
- [iSEE](https://bioconductor.org/packages/release/bioc/html/iSEE.html)
  is a Bioconductor package that provides an interactive Shiny-based
  graphical user interface for exploring data stored in
  `SummarizedExperiment` objects [@iSEE:2018].
- [Glimma](https://www.bioconductor.org/packages/release/bioc/html/Glimma.html)
  is a Bioconductor package that generates interactive visualisations
  for analysis of RNA-sequencing data using output from
  [limma](https://bioconductor.org/packages/release/bioc/html/limma.html)
  [@limma:2015], *edgeR* or *DESeq2* packages in an HTML page. The
  interactions are built on top of the popular static representations
