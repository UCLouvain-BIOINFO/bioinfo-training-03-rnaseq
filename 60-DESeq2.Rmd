...
- to understand the different theorical concepts behind a differential
  expression analysis
- to provide a real-life example of DE analysis analysis running
  DESeq2 step-by-step
...
below. Briefly, DESeq2 starts by estimating scaling factors. Then, it
estimates the gene-wise dispersions and shrinks these estimates to
...
to the $µ$ parameter, an extra parameter $α$ called the **dispersion**
...
We will run a DESeq2 analysis using the real dataset that we stored previously
as a SummarizedExperiment in our `se` object.
Remember this dataset corresponds to RNA-seq data from two different cell types
...
We now switch our previous `SummarizedExperiment` object over to a `DESeqDataSet`
which is a subclass of `RangedSummarizedExperiment` used by the DESeq2 package
to store the read counts and the intermediate estimated quantities during
statistical analysis.

The `DESeqDataSet` class enforces non-negative integer values in the count
matrix. In addition, a formula which specifies the design of the experiment
(the variables that will be used in modeling) must be provided.
...
the DESeq() function, but by removing rows containing only zeros, we reduce the
...
It is highly recommended to starts by a PCA to have a global view on the data.
It will allow us to assess:
...
- Whether there are any sample outliers that may need further investigation
...
This is important because RNA-seq data are intrinsically heteroscedastic.
In other words, the variance is not constant across the range of expression values.
Genes with higher expression levels generally exhibit greater variance
(left panel in the figure below). Therefore, if raw counts are used as input for PCA,
the result will be dominated by highly expressed genes, as these genes contribute
most to the overall variance.

A simple strategy to address this issue would be to take the logarithm of the
normalised counts. However, log-transformed counts still exhibit heteroscedasticity
(middle panel).

As a solution, DESeq2 offers the varianceStabilizingTransformation , `vst()`,
which stabilizes the variance across the range of mean expression values (right
panel). For genes with high counts, the transformation does not differ much from
an ordinary log2 transformation. For genes with low expression, it applies
stronger variance stabilization, which reduces the high variability associated
with small counts and Poisson noise.
See `?vst` for more details about the function.
...
Note that the argument `returnData = TRUE` can be used to obtain a dataframe of
PC1 and PC2 for custom plotting.
...
Try to reproduce the previous dispersion plot using values from the rowData of dds.

Hints:

- the means of normalized counts are stored in the `baseMean` column
- Gene initial dispersion values are stored in the `dispGeneEst` column
...
The `name` parameter must be an element of resultsNames(object)
specifying the samples to compare.
...
How would you use the results() function to test the KD effect in fibroblast
cells? Try to do it in two different ways: one using the `contrast` argument
and one using the `name` argument.
...
Using the resultsNames(), we can see that there is not coefficient corresponding
to `group_Fibroblast_KD_vs_Fibroblast_ctl`. This is not surprising because the
reference level of the group factor is `Epithelial_ctl`.
...
In this case, we should first set the `Fibroblast_ctl` group as the reference
group. Reference levels can be changed using the `relevel()` function. But in
this case don't forget to re-run `DESeq()` function after the
...
the "Condition_KD_vs_ctl" coefficient, positive log2FC indicates a gene up-regulated
in the KD condition compared to the control condition, while a negative log2FC
indicate a down-regulation.
...
Inspect the results table and identify the "best gene" (showing
...
```{r}
# Saving result table for next chapter
...
- [Glimma](https://www.bioconductor.org/packages/release/bioc/html/Glimma.html)
  ...
  for analysis of RNA-sequencing data using output from limma, edgeR
  or DESeq2 packages in an HTML page. The interactions are built on
  top of the popular static representations of analysis results in
