...
which stabilizes the variance across the range of mean expression values (right panel).
For genes with high counts, the transformation does not differ much from an
ordinary log2 transformation. For genes with low expression, it applies stronger variance stabilization, which reduces the high variability associated with small counts and Poisson noise.
See `?vst` for more details about the function.

```{r echo=FALSE, warning=FALSE}
dds_group1 <- dds[, dds$group == "Epithelial_mock"]
...
based on alphabetical order. Here `Epithelial_KD` was hence set
...
Reference levels can be changed using the `relevel()` function. But in
this case don't forget to re-run `DESeq()` function after the
re-leveling operation.

```{r, message = FALSE}
dds$group <- relevel(dds$group, ref = "Epithelial_mock")
dds <- DESeq(dds)
resultsNames(dds)
```
...
```{r}
res <- results(dds,
               name = "group_Epithelial_KD_vs_Epithelial_mock")
```
...
```{r}
res <- results(dds,
               contrast = c("group", "Epithelial_KD", "Epithelial_mock"))
```
...
How would you run the results() function to identify genes affected by the KD in
fibroblast cells?
...
```{r}
results(dds,
        contrast = c("group", "Fibroblast_KD", "Fibroblast_mock"))
```
...
of the "best gene" in mock cells. Compare with ß coefficients.

<details>
<summary>Solution</summary>

```{r}
# log2 expression level in mock cells
log2(rowMeans(counts(dds[best_gene, dds$group == "Epithelial_mock"], normalize = TRUE)))
```
...
This means that genes whith basemean < `r round(metadata(res)$filterThreshold,2)` have been
filtered. This represents `r round(as.numeric(sub("%", "", names(metadata(res)$filterThreshold))), 1)` of all tested genes!
...
```{r, warning = FALSE, message = FALSE}
res_shr <- lfcShrink(dds, coef = "group_Epithelial_KD_vs_Epithelial_mock")
